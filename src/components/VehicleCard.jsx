...
const VehicleCard = ({ vehicle }) => {
  const { id, images, name, brand, year, category, transmission, seats, fuel, price } = vehicle
  ...
  return (
    <div className="group bg-surface border border-border rounded-2xl overflow-hidden hover:border-gold/50 transition-colors duration-300">
      <Link to={`/fleet/${id}`} className="block aspect-[4/3] overflow-hidden">
        <img
          src={images[0]}
          alt={`${brand} ${name}`}
      ...
      <div className="p-5">
        <div className="flex items-start justify-between gap-2">
          <div>
            <p className="text-xs uppercase tracking-wider text-gold">{category}</p>
        ...
        <div className="flex items-center justify-between mt-5 pt-4 border-t border-border">
          <div>
            ...
            <p className="text-xs text-foreground-subtle">per day</p>
          </div>
          <Link
            to={`/fleet/${id}`}
